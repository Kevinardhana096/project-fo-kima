...
async fn get_token() -> Result<String, String> {
    ...
    let secret = read_service_account_key(cred_path)
        .await
...
pub async fn sync_pelanggan_to_sheets(pool: &MySqlPool) -> Result<(), String> {
    let spreadsheet_id =
    ...
    for (i, row) in rows.into_iter().enumerate() {
        ...
        row_data.push(
            row.try_get::<Option<String>, _>("kode_pelanggan")
                .unwrap_or_default()
                .unwrap_or_default(),
        );
        row_data.push(
            row.try_get::<String, _>("nama_pelanggan")
                .unwrap_or_default(),
        );
        row_data.push(
            row.try_get::<i64, _>("kontrak_aktif")
                .unwrap_or(0)
                .to_string(),
        );
        row_data.push(
            row.try_get::<Option<String>, _>("pic")
                .unwrap_or_default()
                .unwrap_or_default(),
        );
        row_data.push(
            row.try_get::<Option<String>, _>("telepon")
                .unwrap_or_default()
                .unwrap_or_default(),
        );
        row_data.push(
    ...
    let clear_url = format!(
        "https://sheets.googleapis.com/v4/spreadsheets/{}/values/Pelanggan!A2:J:clear",
        spreadsheet_id
    );
    ...
    if !values.is_empty() {
        let append_url = format!(
            "https://sheets.googleapis.com/v4/spreadsheets/{}/values/Pelanggan!A2:append?valueInputOption=USER_ENTERED",
            spreadsheet_id
        );

        let append_req = AppendRequest {
            range: "Pelanggan!A2".to_string(),
...
pub async fn sync_lokasi_to_sheets(pool: &MySqlPool) -> Result<(), String> {
    ...
    for (i, row) in rows.into_iter().enumerate() {
        let mut row_data = Vec::new();
        row_data.push((i + 1).to_string()); // No
        row_data.push(row.try_get::<String, _>("kode_kontrak").unwrap_or_default());
        row_data.push(
            row.try_get::<Option<String>, _>("nama_pelanggan")
                .unwrap_or_default()
                .unwrap_or_default(),
        );
        row_data.push(row.try_get::<String, _>("kategori").unwrap_or_default());
        row_data.push(row.try_get::<String, _>("nama_lokasi").unwrap_or_default());

        let core = row
            .try_get::<Option<String>, _>("core")
            .unwrap_or_default()
            .unwrap_or_default();
        let sharing = row
            .try_get::<Option<String>, _>("sharing_core")
            .unwrap_or_default()
            .unwrap_or_default();
        ...
        row_data.push(row.try_get::<f64, _>("perbulan").unwrap_or(0.0).to_string());
